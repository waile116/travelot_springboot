    ...
    <div class="container">
      <ul class="select">
        <li class="state">
          <label
            ><p>州属</p>
            <select v-model="state" @change="listAttraction">
              <option
        ...
        <li class="search">
          <input type="text" v-model="query" placeholder="搜索景点..." />
        </li>
...
export default {
  ...
  methods: {
    ...
    listAttraction() {
      this.$axios
        .get(`AttractionController/listAttractionById/${this.state.stateId}`)
        .then((response) => {
          this.attractionArr = response.data.result;
          this.filter = this.attractionArr;
        })
        .catch((error) => {
          console.error(error);
        });
    ...
    search() {
      ...
      if (this.query === "") {
        ...
        this.filter = this.attractionArr.filter((attraction) =>
